...
impl HashWatcher {
    ...
    async fn get_file_hashes_blocking(&self, hash_spec: HashSpec) -> Result<GitHashes, Error> {
...
impl FileHashes {
        ...
        F: FnMut(&AnchoredSystemPath) -> bool,
    {
        ...
        std::mem::swap(&mut self.0, &mut new_trie);
        // rename for clarity now that we've swapped
        let mut previous = new_trie;
...
mod tests {
    ...
    async fn test_basic_file_changes() {
        ...
        let foo_hash = hash_watcher
            .get_file_hashes_blocking(HashSpec {
    ...
    async fn retry_get_hash(
        ...
        while Instant::now() < deadline {
            match hash_watcher.get_file_hashes_blocking(spec.clone()).await {
                Ok(hashes) => {
                    if hashes == expected {
                        return;
                    } else {
